...
def get_data(mf_path=False, em_path=False):
    ...
    if not mf_error:
        if isinstance(millennium_falcon['autonomy'], int):
        ...
        if not db_error:
            ...
            if not em_error:
                if isinstance(empire['countdown'], int):
                    countdown = empire['countdown']
                else:
                    em_error = True
                    print('"Countdown" value is not an integer')
                ...
                for i in range(len(empire['bounty_hunters'])):
                    if empire['bounty_hunters'][i]['planet'] in planets and isinstance(empire['bounty_hunters'][i]['day'], int):
                        ...
                        em_error = True
                        print('bounty_hunter "planet" is not in the database planet list OR '
